...
void orderedLinkedList<Type>::deleteNode(const Type& deleteItem)
{
    ...
    else
    {
        ...
        if (current == NULL)   //Case 4; The node to be deleted is not in the list
            cout << "The item to be deleted is not in the list." << endl;
        else
        {
            if (current->info == deleteItem) //the item to be deleted is in the list
            {
                if (linkedListType<Type>::first == current)       //Case 2; The node to be deleted is the first node
                {
                ...
                else     //Case 3; The node to be deleted is not the first node, it is somewhere in the list
                {
