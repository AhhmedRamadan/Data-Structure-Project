...
public:
    ...
    void deleteNode(const Type& deleteItem);
	void updateAt(int loc,const Type& newItem) ;
};

template <class Type>
void unorderedLinkedList<Type>::
updateAt(int loc,const Type& newItem)
{

    nodeType<Type> * temp;

    if (linkedListType<Type>::first == NULL)
        cout << "cannot update empty list";
    else
    {
...
template <class Type>
void unorderedLinkedList<Type>::
insertAt( int loc, const Type& item)
{
	if (loc < 0 || loc > linkedListType<Type>::count)
	...
	else
	{
		...
		if (loc == 0) //insert at the begining
			insertFirst(item);
		else if (loc == linkedListType<Type>::count) //insert at the end;
			insertLast(item);
		else
		{
			nodeType<Type> *temp = linkedListType<Type>::first;
			for (int i = 1; i < loc; i++)
				temp = temp->link;
...
template <class Type>
void unorderedLinkedList<Type>::
deleteNode(const Type& deleteItem)
{
    nodeType<Type>* current; //pointer to traverse the list
    ...
    {
        if (linkedListType<Type>::first->info == deleteItem) //Case 2; The node to be deleted is the first node
        {
